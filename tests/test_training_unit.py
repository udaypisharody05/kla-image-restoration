...
# --- Experiment 12: NAFNet-SR checkpoint/resume/loading compatibility ---


def _tiny_nafnet_sr_config() -> dict:
    return build_model_config(
        "nafnet_sr", num_features=4, num_blocks=1, scale=2, dw_expand=2, ffn_expand=2
    )


def test_nafnet_sr_checkpoint_stores_architecture_identifier(tmp_path: Path) -> None:
    model_config = _tiny_nafnet_sr_config()
    model = NAFNetSR(in_channels=1, out_channels=1, num_features=4, num_blocks=1, scale=2)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp12_checkpoint.pt"
    save_checkpoint(
        checkpoint_path,
        model,
        optimizer,
    ...
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    assert checkpoint["model_config"]["architecture"] == "nafnet_sr"
    assert checkpoint["model_config"]["dw_expand"] == 2
    assert checkpoint["model_config"]["ffn_expand"] == 2


def test_matching_nafnet_sr_resume_succeeds(tmp_path: Path) -> None:
    model_config = _tiny_nafnet_sr_config()
    model = NAFNetSR(in_channels=1, out_channels=1, num_features=4, num_blocks=1, scale=2)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp12_checkpoint.pt"
    ...
    resumed_model = NAFNetSR(in_channels=1, out_channels=1, num_features=4, num_blocks=1, scale=2)
    resumed_optimizer = torch.optim.Adam(resumed_model.parameters(), lr=1e-4)
    start_epoch, best_val_psnr, _ = load_checkpoint_for_resume(
        checkpoint_path, resumed_model, resumed_optimizer, model_config, torch.device("cpu")
    )
...
def test_residual_sr_checkpoint_rejects_nafnet_sr_resume(tmp_path: Path) -> None:
    """Real Exp1-8-shaped checkpoint (ResidualSRNet, no architecture key) must
    reject a --model nafnet_sr resume attempt."""
    residual_sr_config = _exp3_capacity_model_config()
    model = ResidualSRNet(**residual_sr_config)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp6_checkpoint.pt"
    ...
    nafnet_sr_config = build_model_config(
        "nafnet_sr",
        num_features=residual_sr_config["num_features"],
        num_blocks=residual_sr_config["num_blocks"],
        scale=residual_sr_config["scale"],
    )
    nafnet_model = NAFNetSR(
        num_features=residual_sr_config["num_features"],
        num_blocks=residual_sr_config["num_blocks"],
        scale=residual_sr_config["scale"],
    )
    nafnet_optimizer = torch.optim.Adam(nafnet_model.parameters(), lr=1e-4)
    with pytest.raises(ValueError, match="model_config"):
        load_checkpoint_for_resume(
            checkpoint_path, nafnet_model, nafnet_optimizer, nafnet_sr_config, torch.device("cpu")
        )


def test_nafnet_sr_checkpoint_rejects_residual_sr_resume(tmp_path: Path) -> None:
    nafnet_sr_config = _tiny_nafnet_sr_config()
    model = NAFNetSR(in_channels=1, out_channels=1, num_features=4, num_blocks=1, scale=2)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp12_checkpoint.pt"
    save_checkpoint(
...
def test_edsr_lite_checkpoint_rejects_nafnet_sr_resume(tmp_path: Path) -> None:
    edsr_lite_config = _tiny_edsr_lite_config()
    model = EDSRLite(
        in_channels=1, out_channels=1, num_features=4, num_blocks=1, scale=2, residual_scale=0.1
    )
    ...
    nafnet_model = NAFNetSR(in_channels=1, out_channels=1, num_features=4, num_blocks=1, scale=2)
    nafnet_optimizer = torch.optim.Adam(nafnet_model.parameters(), lr=1e-4)
    with pytest.raises(ValueError, match="model_config"):
        load_checkpoint_for_resume(
            checkpoint_path, nafnet_model, nafnet_optimizer, nafnet_sr_config, torch.device("cpu")
...
def test_nafnet_sr_checkpoint_rejects_edsr_lite_resume(tmp_path: Path) -> None:
    nafnet_sr_config = _tiny_nafnet_sr_config()
    model = NAFNetSR(in_channels=1, out_channels=1, num_features=4, num_blocks=1, scale=2)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp12_checkpoint.pt"
...
def test_evaluate_checkpoint_load_model_reconstructs_nafnet_sr(tmp_path: Path) -> None:
    """Covers both evaluate_checkpoint.py and infer_test.py, which reuses load_model()."""
    model_config = _tiny_nafnet_sr_config()
    model = NAFNetSR(in_channels=1, out_channels=1, num_features=4, num_blocks=1, scale=2)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    ...
    loaded_model, checkpoint = load_model(checkpoint_path, torch.device("cpu"))
    assert isinstance(loaded_model, NAFNetSR)
    assert checkpoint["model_config"]["architecture"] == "nafnet_sr"
    output = loaded_model(torch.randn(1, 1, 16, 16))
    assert output.shape == (1, 1, 32, 32)


# --- Experiment 13: SwinIR-lite checkpoint/resume/loading compatibility ---


def _tiny_swinir_lite_config() -> dict:
    return build_model_config(
        "swinir_lite", embed_dim=8, depth=2, num_heads=2, window_size=4, mlp_ratio=2.0, scale=2
    )


def _tiny_swinir_lite_model() -> SwinIRLite:
    return SwinIRLite(
        in_channels=1, out_channels=1, embed_dim=8, depth=2, num_heads=2, window_size=4,
        mlp_ratio=2.0, scale=2,
    )


def test_swinir_lite_checkpoint_stores_architecture_identifier(tmp_path: Path) -> None:
    model_config = _tiny_swinir_lite_config()
    model = _tiny_swinir_lite_model()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp13_checkpoint.pt"
    save_checkpoint(
        checkpoint_path,
        model,
        optimizer,
    ...
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    assert checkpoint["model_config"]["architecture"] == "swinir_lite"
    assert checkpoint["model_config"]["window_size"] == 4


def test_matching_swinir_lite_resume_succeeds(tmp_path: Path) -> None:
    model_config = _tiny_swinir_lite_config()
    model = _tiny_swinir_lite_model()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp13_checkpoint.pt"
    ...
    resumed_model = _tiny_swinir_lite_model()
    resumed_optimizer = torch.optim.Adam(resumed_model.parameters(), lr=1e-4)
    start_epoch, best_val_psnr, _ = load_checkpoint_for_resume(
        checkpoint_path, resumed_model, resumed_optimizer, model_config, torch.device("cpu")
    )
...
def test_residual_sr_checkpoint_rejects_swinir_lite_resume(tmp_path: Path) -> None:
    residual_sr_config = _exp3_capacity_model_config()
    model = ResidualSRNet(**residual_sr_config)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp6_checkpoint.pt"
    ...
    swinir_lite_config = build_model_config(
        "swinir_lite", embed_dim=8, depth=2, num_heads=2, window_size=4, mlp_ratio=2.0, scale=2
    )
    swinir_model = _tiny_swinir_lite_model()
    swinir_optimizer = torch.optim.Adam(swinir_model.parameters(), lr=1e-4)
    with pytest.raises(ValueError, match="model_config"):
        load_checkpoint_for_resume(
            checkpoint_path, swinir_model, swinir_optimizer, swinir_lite_config, torch.device("cpu")
        )


def test_swinir_lite_checkpoint_rejects_residual_sr_resume(tmp_path: Path) -> None:
    swinir_lite_config = _tiny_swinir_lite_config()
    model = _tiny_swinir_lite_model()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp13_checkpoint.pt"
    save_checkpoint(
        ...
        epoch=5,
        best_val_psnr=20.0,
        model_config=swinir_lite_config,
        training_config={},
    )
...
def test_edsr_lite_checkpoint_rejects_swinir_lite_resume(tmp_path: Path) -> None:
    edsr_lite_config = _tiny_edsr_lite_config()
    model = EDSRLite(
        in_channels=1, out_channels=1, num_features=4, num_blocks=1, scale=2, residual_scale=0.1
    )
...
def test_swinir_lite_checkpoint_rejects_edsr_lite_resume(tmp_path: Path) -> None:
    swinir_lite_config = _tiny_swinir_lite_config()
    model = _tiny_swinir_lite_model()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp13_checkpoint.pt"
...
def test_nafnet_sr_checkpoint_rejects_swinir_lite_resume(tmp_path: Path) -> None:
    nafnet_sr_config = _tiny_nafnet_sr_config()
    model = NAFNetSR(in_channels=1, out_channels=1, num_features=4, num_blocks=1, scale=2)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp12_checkpoint.pt"
...
def test_swinir_lite_checkpoint_rejects_nafnet_sr_resume(tmp_path: Path) -> None:
    swinir_lite_config = _tiny_swinir_lite_config()
    model = _tiny_swinir_lite_model()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp13_checkpoint.pt"
...
def test_evaluate_checkpoint_load_model_reconstructs_swinir_lite(tmp_path: Path) -> None:
    """Covers both evaluate_checkpoint.py and infer_test.py, which reuses load_model()."""
    model_config = _tiny_swinir_lite_config()
    model = _tiny_swinir_lite_model()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    ...
    loaded_model, checkpoint = load_model(checkpoint_path, torch.device("cpu"))
    assert isinstance(loaded_model, SwinIRLite)
    assert checkpoint["model_config"]["architecture"] == "swinir_lite"
    output = loaded_model(torch.randn(1, 1, 16, 16))
    assert output.shape == (1, 1, 32, 32)


# --- Experiment 17: bicubic-residual checkpoint/resume/loading compatibility ---


def _tiny_residual_sr_bicubic_config() -> dict:
    return build_model_config("residual_sr_bicubic", num_features=4, num_blocks=1, scale=2)


def _tiny_residual_sr_bicubic_model() -> ResidualSRBicubic:
    return ResidualSRBicubic(in_channels=1, out_channels=1, num_features=4, num_blocks=1, scale=2)


def test_residual_sr_bicubic_checkpoint_stores_architecture_identifier(tmp_path: Path) -> None:
    model_config = _tiny_residual_sr_bicubic_config()
    model = _tiny_residual_sr_bicubic_model()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp17_checkpoint.pt"
    save_checkpoint(
        checkpoint_path,
        model,
        optimizer,
...
def test_matching_residual_sr_bicubic_resume_succeeds(tmp_path: Path) -> None:
    model_config = _tiny_residual_sr_bicubic_config()
    model = _tiny_residual_sr_bicubic_model()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp17_checkpoint.pt"
    ...
    resumed_model = _tiny_residual_sr_bicubic_model()
    resumed_optimizer = torch.optim.Adam(resumed_model.parameters(), lr=1e-4)
    start_epoch, best_val_psnr, _ = load_checkpoint_for_resume(
        checkpoint_path, resumed_model, resumed_optimizer, model_config, torch.device("cpu")
    )
...
def test_residual_sr_checkpoint_rejects_residual_sr_bicubic_resume(tmp_path: Path) -> None:
    """Real Exp1-8-shaped checkpoint (ResidualSRNet, no architecture key) must
    reject a --model residual_sr_bicubic resume attempt, even though the
    underlying learned-branch tensor shapes are identical."""
    residual_sr_config = _exp3_capacity_model_config()
    model = ResidualSRNet(**residual_sr_config)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp6_checkpoint.pt"
    ...
    bicubic_config = build_model_config(
        "residual_sr_bicubic",
        num_features=residual_sr_config["num_features"],
        num_blocks=residual_sr_config["num_blocks"],
        scale=residual_sr_config["scale"],
    )
    bicubic_model = ResidualSRBicubic(
        num_features=residual_sr_config["num_features"],
        num_blocks=residual_sr_config["num_blocks"],
        scale=residual_sr_config["scale"],
    )
    bicubic_optimizer = torch.optim.Adam(bicubic_model.parameters(), lr=1e-4)
    with pytest.raises(ValueError, match="model_config"):
        load_checkpoint_for_resume(
            checkpoint_path, bicubic_model, bicubic_optimizer, bicubic_config, torch.device("cpu")
        )


def test_residual_sr_bicubic_checkpoint_rejects_residual_sr_resume(tmp_path: Path) -> None:
    bicubic_config = _tiny_residual_sr_bicubic_config()
    model = _tiny_residual_sr_bicubic_model()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    checkpoint_path = tmp_path / "exp17_checkpoint.pt"
    save_checkpoint(
...
def test_evaluate_checkpoint_load_model_reconstructs_residual_sr_bicubic(tmp_path: Path) -> None:
    """Covers both evaluate_checkpoint.py and infer_test.py, which reuses load_model()."""
    model_config = _tiny_residual_sr_bicubic_config()
    model = _tiny_residual_sr_bicubic_model()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    ...
    loaded_model, checkpoint = load_model(checkpoint_path, torch.device("cpu"))
    assert isinstance(loaded_model, ResidualSRBicubic)
    assert checkpoint["model_config"]["architecture"] == "residual_sr_bicubic"
    output = loaded_model(torch.randn(1, 1, 16, 16))
    assert output.shape == (1, 1, 32, 32)
